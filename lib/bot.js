  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        if (text) {
          this.sayOnSlack(channelName, `<${user.name}> ${text}`);
        }

        if (message.files) {
          message.files.forEach(fileInfo => {
            const fileName = fileInfo.title || fileInfo.name;
            const fileURL = fileInfo.url_private;
            const msgPrefix = `* ${user.name} uploaded "${fileName}": `;
            ...
            if (this.sharedFiles.makePublic && this.sharedFiles.nonBotToken) {
              ...
              tempClient.files.sharedPublicURL(fileInfo.id, function(err, res) {
                if (err) {
                  logger.error('Failed to make file public: ', err);
                } else {
                  this.sayOnSlack(channelName,
                                  `${msgPrefix}${res.file.permalink_public}`);
                }
              }.bind(this));
            } else {
              this.sayOnSlack(channelName,
                              `${msgPrefix}${fileURL} (Slack only)`);
            }
          });
        }
