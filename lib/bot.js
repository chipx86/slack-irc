...
const ALLOWED_SUBTYPES = ['file_share', 'me_message'];
...
class Bot {
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        let fileInfo = message.file;
        let fileName = fileInfo.title || fileInfo.name;
        let fileURL = fileInfo.url_private;

        if (this.sharedFiles.makePublic && this.sharedFiles.nonBotToken) {
          const tempClient = new WebClient(this.sharedFiles.nonBotToken);
          tempClient.files.sharedPublicURL(fileInfo.id, function(err, res) {
            if (err) {
              logger.debug('Failed to make file public: ', err);
            } else {
              this.sayOnSlack(
                channelName,
                `* ${user.name} uploaded "${fileName}": ${res.file.permalink_public}`);
            }
          }.bind(this));
        ...
        text = `* ${user.name} uploaded "${fileName}": ${fileURL} (Slack only)`;
      }

      this.sayOnSlack(channelName, text);
    }
  }

  sayOnSlack(channelName, text) {
    logger.debug('Sending message to IRC', channelName, text);
    this.ircClient.say(this.channelMapping[channelName], text);
  }
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      if (author !== this.nickname) {
        iconUrl = this.findGravatar(author);
      }

      if (!iconUrl && author !== this.nickname && this.avatarUrl) {
